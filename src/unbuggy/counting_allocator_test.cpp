/// \file counting_allocator_test.cpp
///
...
typedef unbuggy::counting_allocator<T> X;
typedef unbuggy::counting_allocator<U> Y;

typedef std::allocator_traits<X> XX;
typedef std::allocator_traits<Y> YY;

typedef std::allocator_traits<std::allocator<T> > A;
    // convenient for testing counting_allocator
...
#undef CHECK_TYPE
    ...
    (void)w;                    // Suppress unused variable warnings.
    (void)r;
    (void)t;
    (void)s;
    (void)z;
...
{
    // A default-constructed counting_allocator must default-construct its
    // underlying allocator.

    std::allocator<T>             a0;
    X                             a;        assert(a.get_allocator() == a0);
    unbuggy::counting_allocator<T, X> b;        assert(b.get_allocator() == a);

    // A value-constructed counting_allocator must wrap the supplied allocator.
    ...
    typedef unbuggy::counting_allocator<C> Z;
